  ...
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  ...
  if (!node->has_parameter("goal_blackboard_id")) {
    node->declare_parameter("goal_blackboard_id", std::string("start_point"));
  }
  ...
  goal_sub_ = node->create_subscription<geometry_msgs::msg::PoseStamped>(
    ...
    std::bind(&CleanRackNavigator::onGoalPoseReceived, this, std::placeholders::_1));
  return true;
}

std::string
CleanRackNavigator::getDefaultBTFilepath(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  std::string default_bt_xml_filename;
  auto node = parent_node.lock();
  
  if (!node->has_parameter("default_clean_rack_bt_xml")) {
...
bool
CleanRackNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
...
void
CleanRackNavigator::goalCompleted(typename ActionT::Result::SharedPtr /*result*/)
{
}

void
CleanRackNavigator::onLoop()
{
...
void
CleanRackNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
...
void
CleanRackNavigator::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal)
{
  RCLCPP_INFO(
    logger_, "Begin navigating from current location to (%.2f, %.2f) frame %s %s",
    goal->pose.pose.position.x, goal->pose.pose.position.y, goal->pose.header.frame_id.c_str(), "start_point");
  ...
  blackboard->set<geometry_msgs::msg::PoseStamped>("start_point", goal->pose);
}

void
CleanRackNavigator::onGoalPoseReceived(const geometry_msgs::msg::PoseStamped::SharedPtr pose)
{
  RCLCPP_INFO(logger_, "Start point callback clean_rack");
  ActionT::Goal goal;
  goal.pose = *pose;
