...
class DriveOnHeading : public Recovery<ActionT>
{
public:
  /**
  ...
  Status onRun(const std::shared_ptr<const typename ActionT::Goal> command) override
  {
    ...
    if (!((command->target.x > 0.0) == (command->speed > 0.0)) ) {
      RCLCPP_ERROR(this->logger_, "Speed and command sign did not match %f %f", command->target.x, command->speed);
      return Status::FAILED;
    }
...
protected:
    ...
    geometry_msgs::msg::Pose2D & pose2d)
  {
    ...
    while (cycle_count < max_cycle_count) {
      ...
      if (!this->collision_checker_->isCollisionFree(pose2d)) {
        return false;
      }
      // fetch_data = false;
    }
